...
| S3（固定・拡張用） | 固定項目テーブル（`s_fixed_items[]`）。行数は配列長から算出し、項目追加だけで拡張可能。「会話履歴」「タイマー」「ストップウォッチ」の3項目 |
...
- 「タイマー」Select — 既存タイマーが1件もなければタイマー設定画面（3.5）へ自動遷移。
  既存タイマーがあれば最初の1件の ActionMenu（3.4）を開く（複数ある場合も先頭のみ対象）。
- 「ストップウォッチ」Select — 既存 SW が1件もなければボタン操作のみで即座に開始
  （ラベルなし）。既存 SW があれば最初の1件の ActionMenu（3.4）を開く。
