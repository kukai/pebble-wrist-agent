...
| SELECT 長押し (700 ms) | 分秒ピッカー（3.5）を現在の長さで開き、時間を設定し直す（ADR-028） | リセット（0秒・停止状態へ、Lap履歴もクリア） |
...
- **タイマーの「リセット」は分秒ピッカーを開く方式に変更**（ADR-028。
  ...
  時間を設定し直せないとの指摘を受けた）。SELECT長押しで、そのタイマーの
  現在の長さ（分秒）をプリセットした状態で分秒ピッカー（3.5）→確認画面
  （3.5b）を開く。確認画面で値を変えずに決定すれば実質的に「元の長さへの
  リセット」として機能し、値を変えれば新しい時間の再設定になる。決定時は
  既存スロットの `duration`/`remaining` を書き換えるだけで、Wakeup 予約は
  次に SELECT でトグルするまで行わない（停止状態で待機）。SW のリセットは
  ADR-027 のとおり即座に0秒・停止状態に戻る（変更なし）。
