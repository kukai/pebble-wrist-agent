...
| SELECT 長押し (700 ms) | リセット（即時。停止状態・元の長さに戻す） | リセット（即時。0秒・停止状態へ、Lap履歴もクリア） |
| UP 短押し | （割当なし） | Lap（実行中のみ記録、停止中/タイマーは無視。履歴として複数件保持） |
| DOWN 短押し | 時間の再設定（分秒ピッカー3.5をデフォルト値で開く） | Lap 一覧（3.5b SCREEN_LAPS）を開く |
| BACK 短押し | HOME へ戻る（状態は保持） | 同左 |
| BACK 長押し (700 ms) | 削除して HOME へ | 同左 |

- タイマーの一時停止 = Wakeup キャンセル + 残り秒保存。再開 = 残り秒で再スケジュール。
- **「リセット」と「時間の再設定」は別々の操作として明確に分離している**
  （ADR-032）。SELECT長押しの「リセット」は画面遷移なしにその場で即座に
  確定する（ADR-027 の「停止状態・元の長さに戻すだけ・自動再開しない」を
  タイマー・SW とも踏襲）。一度リセットボタンを押せばその時点で確定して
  おり、その後 BACK で HOME に戻ってキャンセルする、という余地はない。
  時間そのものを変えたい場合は DOWN から分秒ピッカー（3.5）→確認画面
  （3.5b）を開く。ピッカーは常にデフォルト値（5分0秒）から開始し、既存の
  タイマーの現在の長さはプリフィルしない（ADR-031）。決定時は既存スロット
  の `duration`/`remaining` を書き換えるだけで、Wakeup 予約は次に SELECT
  でトグルするまで行わない（停止状態で待機）。
  - （経緯）ADR-028 で一度「SELECT長押し＝分秒ピッカーを開く」という
    設計にしたが、ピッカーを開いただけでは何も確定しておらず、BACK で
    キャンセルすると元の（一時停止中などの）状態がそのまま残る、という
    「リセットが中途半端」な挙動になっていたため、ADR-032 でリセットと
    時間再設定を別ボタンに分離した。
...
- **SELECT 短押し** — 決定。以下の2パターンに分岐する（ADR-028）。
  - **新規作成**（HOME「タイマー」が未設定だった場合）: `分*60+秒` を
    `handle_timer_set()` にローカル実行で渡す（ラベルなし。音声経由の
    `set_timer` ツールと同じ内部関数を再利用。30秒未満は既存クランプで
    30秒に切り上げ）。既にタイマーが1件ある場合は `handle_timer_set()`
    内部のガードにより作成できず、ステータス行に「タイマーは1つまでです」
    を表示して HOME へ戻る（音声経由でも同じ制限がかかる）。
  - **既存タイマーの時間設定し直し**（SLOT の DOWN から来た場合）:
    既存スロットの `duration`/`remaining` を新しい値に書き換え、停止状態
    にする（Wakeup 予約は行わない）。「タイマーは1つまでです」のガードは
    通らない。
  - いずれも成功時は対象スロットの SLOT 画面（3.4）を開く。
