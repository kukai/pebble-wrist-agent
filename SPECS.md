...
| SELECT 長押し (700 ms) | 分秒ピッカー（3.5）をデフォルト値（5分0秒）で開き、時間を設定し直す（ADR-028/031） | リセット（0秒・停止状態へ、Lap履歴もクリア） |
...
- **タイマーの「リセット」は分秒ピッカーを開く方式に変更**（ADR-028。
  ADR-027 では「停止状態・元の長さに戻すだけ」だったが、それだと新しい
  時間を設定し直せないとの指摘を受けた）。SELECT長押しで分秒ピッカー
  （3.5）→確認画面（3.5b）を開く。ピッカーは**常にデフォルト値
  （5分0秒）から開始し、そのタイマーの現在の長さはプリフィルしない**
  （ADR-031。「リセット」という言葉どおり、まっさらな初期状態から
  設定し直せる方が分かりやすいとの指摘を受け、ADR-028 時点の「現在の
  長さをプリセット」から変更した）。決定時は既存スロットの
  `duration`/`remaining` を書き換えるだけで、Wakeup 予約は次に SELECT
  でトグルするまで行わない（停止状態で待機）。SW のリセットは ADR-027
  のとおり即座に0秒・停止状態に戻る（変更なし）。
...
- **SELECT 短押し** — 決定。以下の2パターンに分岐する（ADR-028）。
  ...
  - **既存タイマーの時間設定し直し**（SLOT の SELECT長押しから来た場合）:
