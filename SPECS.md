...
インスタンス数だけ行が増減する設計から変更）。

| 行 | タイトル | サブタイトル |
|----|---------|-------------|
| 1 | 話す | "Selectで音声入力"（固定） |
| 2 | タイマー | 未設定なら "未設定"。アクティブなら最初に見つかった1件の残り時間 `MM:SS`（1h以上は`H:MM:SS`）+ ラベル（あれば）。2件以上アクティブなら "+N件" を付記 |
| 3 | ストップウォッチ | タイマーと同様（経過時間、Lap があれば "Lap MM:SS" も付記） |
| 4 | 会話履歴 | なし |
| 5 | 天気 | なし |
...
- タイマー/SW 行のサブタイトルはカウンタを持たず、TickTimerService
  ...
  タイムスタンプ差分から都度再計算。
- 「タイマー」Select — アクティブなタイマーが1件もなければタイマー設定画面
  （3.5）へ自動遷移し、作成後はそのタイマーの ActionMenu（3.4）をそのまま
  開く。既にあれば最初に見つかった1件の ActionMenu（3.4）を開く（複数ある
  場合も先頭のみ対象。2件目以降は同じ ActionMenu 内の「新しいタイマー」経由、
  またはその ActionMenu からのみ操作できる）。
- 「ストップウォッチ」Select — 既存 SW が1件もなければボタン操作のみで即座に
  開始（ラベルなし）し、開始後はその SW の ActionMenu（3.4）をそのまま開く。
  既にあれば最初の1件の ActionMenu（3.4）を開く。
...
空きスロットがある場合、末尾に「新しいタイマー」または「新しいストップウォッチ」を追加する。

| 種別 | 状態 | アクション |
|------|------|-----------|
| タイマー | 実行中 | 一時停止 / リセット / 削除 (/ 新しいタイマー) |
| タイマー | 停止中 | 再開 / リセット / 削除 (/ 新しいタイマー) |
| ストップウォッチ | 実行中 | ストップ / Lap / リセット / 削除 (/ 新しいストップウォッチ) |
| ストップウォッチ | 停止中 | スタート / リセット / 削除 (/ 新しいストップウォッチ) |

- タイマーの一時停止 = Wakeup キャンセル + 残り秒保存。再開 = 残り秒で再スケジュール。
  リセット = 元の長さ（duration）で再スケジュール。
- SW の Lap は直近 Lap 秒を記録し「ストップウォッチ」行のサブタイトルに表示。
  リセットは 0 秒・停止状態へ。
- 「新しいタイマー」/「新しいストップウォッチ」選択時は、この ActionMenu が完全に閉じて
  （`did_close`）から次の画面を開く（タイマー設定画面 3.5、または SW の即時開始）。
  ActionMenu の多重起動を避けるため、選択直後ではなく `did_close` まで遅延する。
- タイマー/SW を新規作成した直後は、作成したスロットの ActionMenu をそのまま開く
  （同じ `did_close` 遅延の仕組みを利用）。HOME の「タイマー」/「ストップウォッチ」行
  は増減せず常に1行のままなので、複数アクティブな場合の2件目以降を個別に操作したい
  場合はこの ActionMenu を経由する（3.1 参照）。
- ActionMenu の容量は最大アクション数（SW実行中の5件）に合わせて確保。

### 3.5 タイマー設定画面（プリセットピッカー）

ActionMenu によるプリセット時間選択（数値入力 UI は使わない）。
HOME「タイマー」行（未設定時）、または上記 ActionMenu の「新しいタイマー」から遷移する。

- プリセット: 1分 / 3分 / 5分 / 10分 / 15分 / 20分 / 30分
- 選択したプリセットで `handle_timer_set()` をローカル実行（ラベルなし。音声経由の
  `set_timer` ツールと同じ内部関数を再利用）。空きスロットがなければ画面を開かず
  ステータス行に「タイマー枠満杯」を表示。
