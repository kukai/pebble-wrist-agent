...
| SELECT 長押し (700 ms) | リセット（元の長さで再スケジュール） | リセット（0秒・停止状態へ） |
| UP 短押し | （割当なし） | Lap（実行中のみ記録、停止中/タイマーは無視） |
...
- 分（0–180、循環）・秒（0–59、循環）を左右に大きく表示し、選択中の
  フィールドは反転表示（黒地白文字）でハイライトする。分・コロン・秒の
  3要素はまとめて画面中央（上下左右）に配置し、ハイライトの反転表示は
  数字の幅にぴったり収まる（画面端まで伸びない）（ADR-025。フィールド幅
  70px、ADR-026 で50pxから拡大 — 50pxでは `FONT_KEY_BITHAM_42_BOLD` の
  2桁数字が収まらず省略記号表示になる回帰があったため）。
- **UP / DOWN 短押し** — 選択中フィールドの値を ±1（循環）
- **SELECT 短押し** — 分 ⇄ 秒でフィールド選択を切替
- **SELECT 長押し (700 ms)** — 決定。`分*60+秒` を `handle_timer_set()` に
  渡してローカル実行（ラベルなし。音声経由の `set_timer` ツールと同じ内部
  関数を再利用。30秒未満は既存クランプで30秒に切り上げ）。既にタイマーが
  1件ある場合は `handle_timer_set()` 内部のガードにより作成できず、
  ステータス行に「タイマーは1つまでです」を表示する（音声経由でも同じ
  制限がかかる）。
