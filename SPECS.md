...
| SELECT 長押し (700 ms) | リセット（即時。スロットを完全に削除して未設定に戻し、そのまま分秒ピッカー3.5へ） | リセット（即時。スロットを完全に削除して未設定に戻し、HOMEへ） |
...
- **「リセット」はタイマー・SW とも、スロットを完全に削除して未設定に
  戻す即時確定の操作**（ADR-033。旧「削除」＝BACK長押しと同じ結果になる
  ため、BACK長押しは廃止し SELECT長押しに統合した）。
  - タイマー: 削除した直後、そのまま分秒ピッカー（3.5）→確認画面
    （3.5b）を開き、次の時間をすぐ設定できる（ピッカーは常にデフォルト
    値5分0秒から開始）。タイマーには「動いているか存在しないか」以外の
    自然な待機状態がないため。
  - SW: 削除した直後、HOME へ戻る。SW には設定すべき値がないため、次に
    「ストップウォッチ」を選べば即座に新しい計測を開始できる。
  - どちらも操作した瞬間に確定しており、その後 BACK で HOME に戻っても
    元の状態が復活することはない。
  - （経緯）ADR-027〜032 では「リセット＝停止状態・元の長さ/0秒に戻す
    だけでスロットは残す」という設計を何度か試したが、「HOME に戻って
    再度入ると、リセットしたはずのタイマー/SWがまだ存在している」
    「未設定に戻ってほしい」という指摘を受け、ADR-033 で「リセット＝
    完全に削除」という単純な定義に統一した。
