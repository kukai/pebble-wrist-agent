...
| S3（固定・拡張用） | 固定項目テーブル（`s_fixed_items[]`）。行数は配列長から算出し、項目追加だけで拡張可能。「会話履歴」「タイマー」「ストップウォッチ」「天気」の4項目 |
...
- 「タイマー」Select — 既存タイマーが1件もなければタイマー設定画面（3.5）へ自動遷移し、
  作成後はそのタイマーの ActionMenu（3.4）をそのまま開く。既存タイマーがあれば
  最初の1件の ActionMenu（3.4）を開く（複数ある場合も先頭のみ対象）。
- 「ストップウォッチ」Select — 既存 SW が1件もなければボタン操作のみで即座に開始
  （ラベルなし）し、開始後はその SW の ActionMenu（3.4）をそのまま開く。既存 SW が
  あれば最初の1件の ActionMenu（3.4）を開く。
...
### 3.4 ActionMenu（S2 行選択時、または S3「タイマー」「ストップウォッチ」選択時）
...
- タイマー/SW を新規作成した直後は、S2 に行が増えるだけで終わらせず、作成した
  スロットの ActionMenu をそのまま開く（同じ `did_close` 遅延の仕組みを利用）。
  ユーザーが自分で S2 から選び直す手間を無くすため。
- ActionMenu の容量は最大アクション数（SW実行中の5件）に合わせて確保。

### 3.5 タイマー設定画面（プリセットピッカー）

ActionMenu によるプリセット時間選択（数値入力 UI は使わない）。
S3「タイマー」（未設定時）、または上記 ActionMenu の「新しいタイマー」から遷移する。
