...
| SELECT 長押し (700 ms) | リセット（停止状態・元の長さに戻すのみ。自動再開はしない） | リセット（0秒・停止状態へ、Lap履歴もクリア） |
...
- **リセット直後にタイマーが再スタートすることはない**（ADR-027）。以前は
  リセット時に元の長さで即座に再スケジュールし `running=1` にしていたが、
  「リセットしたら勝手に動き出す」のは意図に反するとの指摘を受け、リセットは
  停止状態・元の長さに戻すだけに変更した。再開したい場合はユーザーが
  改めて SELECT でトグルする。
...
- **SELECT 短押し** — 決定。`分*60+秒` を `handle_timer_set()` にローカル
  ...
  再利用。30秒未満は既存クランプで30秒に切り上げ）。成功時は作成した
  スロットの SLOT 画面（3.4）を開く。既にタイマーが1件ある場合は
  `handle_timer_set()` 内部のガードにより作成できず、ステータス行に
  「タイマーは1つまでです」を表示して HOME へ戻る（音声経由でも同じ
  制限がかかる）。
