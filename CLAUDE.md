...
```
HOME (MenuLayer 単一セクション・固定5行: 話す/タイマー/ストップウォッチ/会話履歴/天気)
  ※ タイマー/ストップウォッチ行はインスタンスの有無・数にかかわらず常に1行のまま。
    状態はサブタイトルで表現（未設定/残り・経過時間/複数なら "+N件"）。ADR-021 参照
...
HOME「タイマー」(SELECT) ──▶ 未設定ならプリセット時間ピッカー(ActionMenu)、
  設定済みなら該当タイマー(先頭1件)の ActionMenu。
  新規作成時は作成したスロットの ActionMenu をそのまま自動的に開く（ADR-020）
HOME「ストップウォッチ」(SELECT) ──▶ 未設定なら即座に開始、
  設定済みなら該当SW(先頭1件)の ActionMenu。
  新規作成時は同様に ActionMenu を自動的に開く（ADR-020）
タイマー/SW の ActionMenu (SELECT) ──▶ 一時停止/再開/リセット/削除+新しいタイマー、
  SWはStart/Stop/Lap/Reset+新しいSW。全てローカル処理。複数アクティブな場合の
  2件目以降もここから操作する
HOME「会話履歴」(SELECT) ──▶ ANSWER（履歴0件なら「履歴なし」表示）
HOME「天気」(SELECT) ──▶ 「今日の天気を教えて」を送信 ──▶ LOADING ──▶ ANSWER
  （「話す」と同じ経路。get_weather ツールが応答。ADR-019）
...
- HOME の「タイマー」/「ストップウォッチ」行はインスタンス数にかかわらず常に1行のまま
  （増減しない）。複数アクティブでもサブタイトルに "+N件" と表示するのみで、行は増やさない
  （ADR-021 参照。旧 S2 の「インスタンスごとに行が増える」設計から変更）。
