...
HOME S3「天気」(SELECT) ──▶ 「今日の天気を教えて」を送信 ──▶ LOADING ──▶ ANSWER
  （S1「話す」と同じ経路。get_weather ツールが応答。ADR-019）
...
- 新規作成直後は S2 に行が増えるだけで終わらせず、作成したスロットの ActionMenu を
  自動的に開く（`s_open_slot_pending` で `did_close` 後に遅延オープン。ADR-020 参照）。

## 天気のローカル送信（ボタン操作のみ、音声非経由）

- HOME S3「天気」から、音声を使わず「今日の天気を教えて」を送信できる（ADR-019 参照）。
